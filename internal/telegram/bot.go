...
func NewBot(token string, chatID int64, stateMgr *state.Manager, claudeMgr *claude.Manager, fileStore *store.FileStore, scheduleMgr *schedule.Manager, embeddedContext string) (*Bot, error) {
	...
	b.api = api
	b.sender = NewSender(api, chatID)
	b.output = NewOutputManager(api, chatID, b.sender)
...
func (b *Bot) Start(ctx context.Context) {
	b.ctx = ctx
	b.sender.SetContext(ctx)
	b.output.SetContext(ctx)

	SetBotCommands(ctx, b.api, b.chatID)

	if me, err := b.api.GetMe(ctx); err == nil {
		log.Info(ctx, "telegram bot started", log.Attr{K: "username", V: me.Username})
	} else {
		log.Info(ctx, "telegram bot started")
	}
...
func (b *Bot) handleTopicClosed(ctx context.Context, threadID int) {
	...
	DeleteForumTopic(ctx, b.api, b.chatID, threadID)
...
func (b *Bot) CreateProject(name, prompt, dir string) (*CreateProjectResult, error) {
	threadID, err := CreateForumTopic(b.ctx, b.api, b.chatID, name)
	if err != nil {
...
func (b *Bot) DeleteProject(projectID string) error {
	...
	if p.ThreadID != 0 {
		...
		DeleteForumTopic(b.ctx, b.api, b.chatID, p.ThreadID)
	}
...
func (b *Bot) replyDM(ctx context.Context, chatID int64) {
	if b.dmSent[chatID] {
		return
	}
...
func (b *Bot) handleVoice(ctx context.Context, msg *models.Message) {
	threadID := msg.MessageThreadID
	SetReaction(ctx, b.api, b.chatID, msg.ID, "✍️")

	path, err := b.downloadFile(ctx, msg.Voice.FileID, "voice.ogg")
	if err != nil {
		SetReaction(ctx, b.api, b.chatID, msg.ID, "")
	...
	SendChatAction(ctx, b.api, b.chatID, threadID, models.ChatActionTyping)
	b.output.SetUserMessage(threadID, msg.ID)
...
func (b *Bot) addToBatch(ctx context.Context, msg *models.Message) {
	threadID := msg.MessageThreadID
	...
	if len(msg.Photo) > 0 {
		...
		if path, err := b.downloadFile(ctx, photo.FileID, "photo.jpg"); err == nil {
			files = append(files, path)
		}
	}
	if msg.Document != nil {
		if path, err := b.downloadFile(ctx, msg.Document.FileID, msg.Document.FileName); err == nil {
			files = append(files, path)
		}
	}
	if msg.Video != nil {
		...
		if path, err := b.downloadFile(ctx, msg.Video.FileID, filename); err == nil {
			files = append(files, path)
		}
	}
	if msg.Audio != nil {
		...
		if path, err := b.downloadFile(ctx, msg.Audio.FileID, filename); err == nil {
			files = append(files, path)
		}
	}
...
func (b *Bot) flushBatch(ctx context.Context, threadID int) {
	...
	SendChatAction(ctx, b.api, b.chatID, threadID, models.ChatActionTyping)
	b.output.SetUserMessage(threadID, lastMsg.ID)
...
func (b *Bot) downloadFile(ctx context.Context, fileID, filename string) (string, error) {
	file, err := b.api.GetFile(ctx, &bot.GetFileParams{FileID: fileID})
	if err != nil {
		return "", err
	}

	resp, err := httpClient.Get(b.api.FileDownloadLink(file))
	if err != nil {
		return "", err
	}
...
func (b *Bot) TriggerSchedule(s schedule.Schedule) {
	...
	SendChatAction(b.ctx, b.api, b.chatID, s.ThreadID, models.ChatActionTyping)
